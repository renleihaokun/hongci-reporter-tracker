...
- 🔍 **交互式大图**：点任意趋势卡打开全屏大图——拖动平移、双指/滚轮缩放、十字线读数、全部/90/30/14 天快捷区间、最高/最低/均值/异常统计
- 🧭 **数据点溯源**：大图中点任意数据点，显示该点的来源报告、审核时间、标本号，一键跳到对应报告卡片；报告列表每行也可反向跳回趋势图并自动定位该点
- 📈 **长期指标自动发现**：自动扫描归档中出现 ≥3 次且数值化的项目（如铁蛋白、电解质、肝功能各项），在「＋ 添加指标」面板中勾选即加入趋势区，选择自动记忆（localStorage）；真实归档项目常达 50+ 项，面板提供**项目名筛选**、默认折叠与「展开全部」，已选中的项目始终排在最前
...
# 用本地 JSON 快照预览（如真实数据导出件，注意勿入库）：
DATA_JSON=.local-data/real-data.json node test/dev-server.mjs
```
